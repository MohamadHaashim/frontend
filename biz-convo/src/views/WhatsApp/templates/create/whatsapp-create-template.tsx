...
const handleQuickButtonOpt = () => {
  ...
  if (total >= 10) return; // 🚫 Stop if total reached 10

  const newBtn = { text: "" };
  setquickreplyButtons([...quickreplyButtons, newBtn]);

  if (total + 1 >= 10) setquickreplyDisabled(true);
...
// const handlequickreplyInputChange = (index:any, field:any, value:any) => {
//   const updated = [...quickreplyButtons];
//   updated[index][field] = value;
//   setquickreplyButtons(updated);

//   const allEmpty = updated.every((btn) => btn.text.trim() === "");
//   if (allEmpty) {
...
const handlequickreplyInputChange = (index:any, field:any, value:any) => {
  setquickreplyButtons((prev:any) => {
    // 1️⃣ Update quickreplyButtons
    const updatedQuickReplies = prev.map((item:any, i:any) =>
      i === index ? { ...item, [field]: value } : item
    );

    // 2️⃣ Update buttonOrder so UI re-renders with new text
    setButtonOrder((prevOrder) =>
      prevOrder.map((btn, i) => {
        if (btn.type === "QUICK_REPLY" && i === index) {
          return {
            ...btn,
            data: { ...btn.data, [field]: value },
          };
        }
        return btn;
      })
    ...
    // 3️⃣ Manage active states
    const allEmpty = updatedQuickReplies.every(
      (btn:any) => btn.text.trim() === ""
    );
    setbuttonActive(!allEmpty);
    setquickbtn(allEmpty ? "" : "QUICK_REPLY");

    return updatedQuickReplies;
  });
};


const removequickreplyButton = (index: number) => {
  const updatedQuickReplies = [...quickreplyButtons];

  const [removedButton] = updatedQuickReplies.splice(index, 1);
  setquickreplyButtons(updatedQuickReplies);

  setButtonOrder((prev) => {
    const quickReplies = prev.filter((b) => b.type === "QUICK_REPLY");
    const others = prev.filter((b) => b.type !== "QUICK_REPLY");

    const updatedQuickRepliesOrder = quickReplies.filter(
      (_, i) => i !== index
    );

    return [...others, ...updatedQuickRepliesOrder];
  });
  ...
  const anyFilled = updatedQuickReplies.some(
    (btn) => btn.text.trim() !== ""
  );
  setbuttonActive(anyFilled);
  setquickbtn(anyFilled ? "QUICK_REPLY" : "");
};
...
const handleurlButtonOpt = () => {    
  ...
  if (totalUrlButtons >= 2) return; // Max 2 URLs

  const newBtn = { text: "", url: "" };
  setUrlButtons([...urlButtons, newBtn]);
  if (totalButtons + 1 >= 10 || totalUrlButtons + 1 >= 2)
...
const handleInputChange = (index: number, field: string, value: string) => {
  setUrlButtons((prev:any) => {
    const updated = prev.map((item:any, i:any) =>
      i === index ? { ...item, [field]: value } : item
    );

    const allEmpty = updated.every(
      (btn:any) => btn.text.trim() === "" && btn.url.trim() === ""
    );

    setbuttonActive(!allEmpty);
    seturlbtn(allEmpty ? "" : "URL");

    return updated;
  });

  // Optional: If your buttonOrder array is used for rendering, update it too
  setButtonOrder((prevOrder) =>
    prevOrder.map((btn, i) => {
      if (btn.type === "URL" && i === index) {
        return { ...btn, data: { ...btn.data, [field]: value } };
      }
      return btn;
    })
  );
...
const removeUrlButton = (index: number) => {
  // 1️⃣ Copy current URL buttons
  const updatedUrlButtons = [...urlButtons];

  // 2️⃣ Remove only the clicked button
  const [removedButton] = updatedUrlButtons.splice(index, 1);

  // 3️⃣ Update state
  setUrlButtons(updatedUrlButtons);

  // 4️⃣ Remove the same indexed URL from buttonOrder (preserve other types)
  setButtonOrder((prev) => {
    const urlBtns = prev.filter((b) => b.type === "URL");
    const otherBtns = prev.filter((b) => b.type !== "URL");

    // Remove the specific URL button
    const remainingUrlBtns = urlBtns.filter((_, i) => i !== index);

    return [...otherBtns, ...remainingUrlBtns];
  });

  // 5️⃣ Recalculate total buttons
  const total =
    updatedUrlButtons.length +
    dynamicurlButtons.length +
    quickreplyButtons.length +
    (buttonPhoneOpt ? 1 : 0) +
    (buttoncopyOpt ? 1 : 0);

  setButtonDisabled(total >= 10);

  // 6️⃣ Update button active state
  const anyFilled = updatedUrlButtons.some(
    (btn) => btn.text.trim() !== "" || btn.url.trim() !== ""
  );
  setbuttonActive(anyFilled);
  seturlbtn(anyFilled ? "URL" : "");
};
// const removeUrlButton = (id: number) => {
//   setUrlButtons((prev:any) => prev.filter((btn:any) => btn.id !== id));
//   setButtonOrder((prev:any) => prev.filter((btn:any) => !(btn.type === "URL" && btn.data.id === id)));
// };
...
const handleDynamicurlButtonOpt = () => {
  if (!canAddMoreButtons()) return;
  if (totalUrlButtons >= 2) return; // Max 2 total URLs

  const newBtn = { text: "", url: "", example: [""] };
  setdynamicUrlButtons([...dynamicurlButtons, newBtn]);
...
const handledynamicInputChange = (index: number, field: string, value: string) => {
  setdynamicUrlButtons((prev:any) => {
    // 1️⃣ Update dynamicurlButtons
    const updated = prev.map((item:any, i:any) => {
      if (i === index) {
        if (field === "example") {
    ...
    setbuttonActive(!allEmpty);
    setdynamicurlbtn(allEmpty ? "" : "URL");

    return updated;
  });

  // 3️⃣ Optional: Update buttonOrder if rendering uses it
  setButtonOrder((prevOrder) =>
    prevOrder.map((btn, i) => {
      if (btn.type === "DYNAMIC_URL" && i === index) {
        if (field === "example") {
          return { ...btn, data: { ...btn.data, example: [value] } };
        }
        return { ...btn, data: { ...btn.data, [field]: value } };
      }
      return btn;
...
const removedynamicUrlButton = (index: number) => {
  // 1️⃣ Make a copy
  const updatedDynamicButtons = [...dynamicurlButtons];

  // 2️⃣ Remove only the clicked button at this index
  updatedDynamicButtons.splice(index, 1);
  setdynamicUrlButtons(updatedDynamicButtons);

  // 3️⃣ Remove only this button from buttonOrder
  setButtonOrder((prev) =>
    prev.filter((b, i) => {
      if (b.type !== "DYNAMIC_URL") return true; // keep other types
      const dynamicButtonsInOrder = prev.filter(btn => btn.type === "DYNAMIC_URL");
      const dynamicIndex = dynamicButtonsInOrder.indexOf(b);
      return dynamicIndex !== index;
    })
  );

  // 4️⃣ Recalculate total buttons
  const total =
    urlButtons.length +
    updatedDynamicButtons.length +
    quickreplyButtons.length +
    (buttonPhoneOpt ? 1 : 0) +
    (buttoncopyOpt ? 1 : 0);
  setButtonDisabled(total >= 10);

  // 5️⃣ Update active state
  const anyFilled = updatedDynamicButtons.some(
    (btn) =>
      btn.text.trim() !== "" ||
      btn.url.trim() !== "" ||
      (btn.example && btn.example[0]?.trim() !== "")
  );
  setbuttonActive(anyFilled);
  setdynamicurlbtn(anyFilled ? "DYNAMIC_URL" : "");
};
...
const handlePhoneButtonOpt = () => {
  if (!canAddMoreButtons()) return;
  if (buttonPhoneOpt) return; // only one phone button

  setButtonPhoneopt(true);
  setbuttonActive(true);
  setphoenobtn("PHONE_NUMBER");
  setButtonOrder((prev) => [...prev, { type: "PHONE_NUMBER"}]);

  if (totalButtons + 1 >= 10) setButtonDisabled(true);
};

const handleCopycodeButtonOpt = () => {
  if (!canAddMoreButtons()) return;
  if (buttoncopyOpt) return; // only one copy code

  setButtoncopyopt(true);
  setbuttonActive(true);
  setcopybtn("COPY_CODE");
  setButtonOrder((prev) => [...prev, { type: "COPY_CODE"}]);
  if (totalButtons + 1 >= 10) setButtonDisabled(true);
};
...
function handleRemoveButton(
  ...
  e.preventDefault();
  setSubmit(true);
  if (!names && !langName && !category) return;

  setIsLoading(true);

  // ✅ Safely gather all button states from `buttonOrder` (latest values)
  const allButtons = buttonOrder
    .map((btn: any) => {
      const data = btn.data || btn; // handle both formats
      switch (btn.type) {
        case "QUICK_REPLY":
          return {
            type: "QUICK_REPLY",
  ...
  // ✅ If buttons exist, mark buttonActive true
    ...
    components: [
      ...
      headerType === "text"
        ? {
            ...
            text: textInput,
            ...(formValue?.name?.trim() && {
              example: { header_text: [formValue?.name.trim()] },
            }),
          }
        : headerType === "image" ||
          headerType === "video" ||
        ...
        ? {
            type: "HEADER",
            format: headerType,
            example: { header_handle: [imgValue] },
          }
        : null,
      ...
      // FOOTER
      footertextInput ? { type: "FOOTER", text: footertextInput } : null,
  ...
  console.log("Final payload before API:", apiData);
  ...
  if (component?.buttons?.length > 0) {
    ...
    component.buttons.forEach((btn: any) => {
      switch (btn.type) {
        case "QUICK_REPLY":
          newButtonOrder.push({ type: "QUICK_REPLY", data: { text: btn.text } });
          break;

        case "PHONE_NUMBER":
          const phoneBtn = {
            type: "PHONE_NUMBER",
            data: { text: btn.text || "", phone_number: btn.phone_number || "" },
          };
          newButtonOrder.push(phoneBtn);
          hasPhone = true;
          setButtonPhoneopt(true);
          setbuttonActive(true);
          setphoenobtn("PHONE_NUMBER");
          setButtonPhonetxt(phoneBtn.data.text);
          setButtonPhoneNotxt(phoneBtn.data.phone_number);
          break;

        case "COPY_CODE":
          const copyCodeBtn = {
            type: "COPY_CODE",
            data: { text: btn.text || "", example: btn.example || [] },
          };
          newButtonOrder.push(copyCodeBtn);
          hasCopy = true;
          setButtoncopyopt(true);
          setbuttonActive(true);
          setcopybtn("COPY_CODE");
          setButtonCopycodetxt(copyCodeBtn.data.text);
          break;
        ...
        case "URL":
          ...
          if (isDynamicUrl) {
            const dynamicUrlBtn = {
              type: "DYNAMIC_URL",
              data: { text: btn.text || "", url: btn.url || "", example: btn.example || [] },
            };
            newButtonOrder.push(dynamicUrlBtn);
            setButtondynamicurlopt(true);
            setbuttonActive(true);
            setdynamicurlbtn("URL");
            setButtondynamicUrltxt(dynamicUrlBtn.data.text);
            setButtondynamicwebUrltxt(dynamicUrlBtn.data.url);
            setButtonexampleUrltxt(dynamicUrlBtn.data.example?.[0] || "");
          } else {
            const staticUrlBtn = { type: "URL", data: { text: btn.text || "", url: btn.url || "" } };
            newButtonOrder.push(staticUrlBtn);
            setButtonurlopt(true);
            setbuttonActive(true);
            seturlbtn("URL");
            setButtonurltxt(staticUrlBtn.data.text);
            setButtonwebUrltxt(staticUrlBtn.data.url);
          }
          break;
    ...
    // Count buttons
    const quickreplyButtons = newButtonOrder.filter((b) => b.type === "QUICK_REPLY");
    const urlButtons = newButtonOrder.filter((b) => b.type === "URL");
    const dynamicurlButtons = newButtonOrder.filter((b) => b.type === "DYNAMIC_URL");
    ...
    // Disable Quick Reply if total >= 10
    setquickreplyDisabled(total >= 10);

    // Disable URL button if there are already 2 URL or 2 Dynamic URL buttons
    const totalUrlCount = urlButtons.length + dynamicurlButtons.length;
    setButtonDisabled(totalUrlCount >= 2 || total >= 10);
  }
  break;
  ...
  {buttonOrder.map((btnItem, index) => {
    ...
    switch (type) {
        ...
        return (
          <div key={index} className="row quick-replybtn mt-3 mb-3">
            ...
            <div
              ...
              // onClick={() => removequickreplyButton(quickreplyButtons.indexOf(data))}
            >
            ...
            <div className="col-md-12 login-input-group">
              ...
              <div className="vendor-create-container">
                <input
                  type="text"
                  autoComplete="off"
                  // onChange={(e) => handlequickreplyInputChange(quickreplyButtons.indexOf(data), "text", e.target.value)}
                   onChange={(e) =>
                    handlequickreplyInputChange(index, "text", e.target.value) // ✅ use direct index
                  }
                  value={data.text}
                  className="vendor-crt-input"
                  placeholder=" "
      ...
      case "PHONE_NUMBER":
        return buttonPhoneOpt ? (
          <div key={index} className="row mt-3 quick-replybtn">
            <div className="col-md-6">
              <p className="text-xs">Phone Number Button</p>
            </div>
            <div
              className="col-md-6 text-end text-xs"
              onClick={() => {
                setButtonOrder((prev) =>
                  prev.filter((b) => b.type !== "PHONE_NUMBER")
                );
            ...
            <div className="col-md-12 login-input-group">
              ...
              <div className="vendor-create-container">
                <input
                  type="text"
                  autoComplete="off"
                  onChange={(e) => setButtonPhonetxt(e.target.value)}
              ...
              <div className="vendor-create-container">
                <input
                  ...
                  autoComplete="off"
                  maxLength={12}
                  onChange={(e) => setButtonPhoneNotxt(e.target.value)}
              ...
              <div className="error-message-required">
                Contact number should start with country code without 0 or +
              </div>
            </div>
          </div>
        ) : null;
      ...
      case "COPY_CODE":
        return buttoncopyOpt ? (
          <div key={index} className="row mt-3 quick-replybtn">
            <div className="col-md-6">
              <p className="text-xs">Coupon Code Copy Button</p>
            </div>
            <div
              className="col-md-6 text-end text-xs"
              onClick={() => {
                // Remove this specific copy button from buttonOrder
                setButtonOrder((prev) => {
                  // keep non-COPY_CODE buttons
                  const others = prev.filter((b) => b.type !== "COPY_CODE");

                  // if multiple copy buttons exist, remove only this index
                  // we assume only 1 button, otherwise you'd store in array
                  return others;
                });
                ...
                // Reset state for this specific button
            ...
            <div className="col-md-12 login-input-group">
              ...
              <div className="vendor-create-container">
                <input
                  type="text"
                  autoComplete="off"
                  onChange={(e) => setButtonCopycodetxt(e.target.value)}
      ...
      case "URL":
        return (
          <div key={index} className="row mt-3 quick-replybtn">
            <div className="col-md-6">
              <p className="text-xs">URL Button</p>
            </div>
            <div
              className="col-md-6 text-end text-xs"
              // onClick={() => removeUrlButton(urlButtons.indexOf(data))}
              onClick={() => removeUrlButton(index)}
            >
              <i className="fa fa-times text-danger cursor-pointer"></i>
            </div>
            <div className="col-md-12 login-input-group">
              ...
              <div className="vendor-create-container">
                <input
                  ...
                  // onChange={(e) => handleInputChange(urlButtons.indexOf(data), "url", e.target.value)}
                  value={data.url}
                  className="vendor-crt-input"
                  placeholder=" "
                  required
      ...
      case "DYNAMIC_URL":
        return (
          <div key={index} className="row mt-3 quick-replybtn">
            <div className="col-md-6">
              <p className="text-xs">Dynamic URL Button</p>
            </div>
            <div
              className="col-md-6 text-end text-xs"
              onClick={() => removedynamicUrlButton(index)}
            >
              <i className="fa fa-times text-danger cursor-pointer"></i>
            </div>
            <div className="col-md-12 login-input-group">
              ...
              <div className="vendor-create-container">
                <input
                  type="text"
                  autoComplete="off"
                  onChange={(e) => handledynamicInputChange(index, "text", e.target.value)}
                  value={data.text}
                  className="vendor-crt-input"
                  placeholder=" "
                  required
              ...
              <div className="vendor-create-container">
                <input
                  type="text"
                  autoComplete="off"
                  // onChange={(e) => handledynamicInputChange(dynamicurlButtons.indexOf(data), "url", e.target.value)}
                  onChange={(e) => handledynamicInputChange(index, "url", e.target.value)}
                ...
                />
                <label htmlFor="vendor-crt-input" className="vendor-crt-label"><i className="fa-solid fa-link"></i></label>
                <p className="staff-passwordInputicon text-sm" style={{top:"9px"}}>{"{{1}}"}</p>
              </div>
              <br />
              <p className="text-xs">Example</p>
              <div className="vendor-create-container">
                <input
                  type="text"
                  autoComplete="off"
                  // onChange={(e) => handledynamicInputChange(dynamicurlButtons.indexOf(data), "example", e.target.value)}
                  onChange={(e) => handledynamicInputChange(index, "example", e.target.value)}
  ...
  {buttonOrder.slice(0, 3).map((btn, index) => {
    switch (btn.type) {
      ...
      case "PHONE_NUMBER":
        return (
          <p key={index} className="template-buttontxt button-option-style text-center">
            <i className="fa-solid fa-phone"></i> {buttonPhonetxt}
          </p>
        );
    ...
    <>
      ...
      <p className="template-buttontxt button-option-style text-center">
        <div className="list-group-item">
          <i className="fa fa-menu"></i> See all options <br />
