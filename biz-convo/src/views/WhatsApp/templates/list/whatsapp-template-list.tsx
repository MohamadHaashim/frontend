         ...
         if (responseData.apiStatus.code === '200') {
            ...
            data?.components.forEach((component: any) => {
               if (component.type === "HEADER") {
                  ...
                  const newUrlbtns: string[] = [];
                  const quickReplybtns: string[] = [];
                  const quickReplytxt: string[] = [];
                  const newButtonurltxts: string[] = [];
                  component?.buttons.forEach((buttonsValue: any) => {
                     if (buttonsValue) {
                        switch (buttonsValue?.type) {
                           case "QUICK_REPLY":
                              quickReplybtns.push("QUICK_REPLY")
                              quickReplytxt.push(buttonsValue?.text);
                           ...
                           case "URL":
                              newUrlbtns.push("URL");
                              newButtonurltxts.push(buttonsValue?.text);
                              break;
                           case "URL":
                  ...
                  setUrlbtns(newUrlbtns);
                  setButtonurltxts(newButtonurltxts);
                  setquickReplybtns(quickReplybtns);
                  setButtonQuicktxt(quickReplytxt);
               }
                                    ...
                                    <div className="conversation-container">
                                       <div className="message received text-start z-0">
                                          ...
                                          <div className='px-3  text-center temp-view-buttons'>
                                             {quickReplybtns.length > 0 && quickReplybtns.map((btn:any, index:any) => (
                                             <p key={index} className="button-option-style template-previewModal-text text-center">
                                                <i className="fa-solid fa-square-arrow-up-right"></i> {buttonQuicktxt[index]}
                                             </p>
                                             ))}
                                             ...
                                             {urlbtns.length > 0 && urlbtns.map((btn:any, index:any) => (
                                                <p key={index} className="button-option-style template-previewModal-text text-center">
                                                   <i className="fa-solid fa-square-arrow-up-right"></i> {buttonurltxts[index]}
                                                </p>
                                             ))}
