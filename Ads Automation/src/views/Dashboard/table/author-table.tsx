...
function AuthorsTable(props) {
  let apiEndPoint = "https://api.aimosa.io/Dashboard/Authors ";
  let pageload = true;
  ...
  const [isperPage, setIsPerPage] = useState<any>();
  ...
  const applyDataLength = (e) => {
    ...
    setIsPerPage(parseInt(e.target.value));
  };
  console.log(isperPage);
    ...
    try {
      ...
      if (columns.length < 1) {
        ...
        for (let i = 0; headers.length > i; i++) {
          if (headers[i]["keyName"] === "roi") {
            columns.push({
              field: headers[i]["keyName"],
              headerName: headers[i]["displayName"],
              width: 124,
              // renderCell: (params) => (
              //   <i className={"status " + params.row.acos}></i>
              // ),
            });
            ...
            columns.push({
              ...
              width: 124,
              // renderCell: (params) => (
              //   <i className={"flag-" + params.row.acos}></i>
              // ),
            });
          } else {
            columns.push({
  ...
  //   // Construct the export payload based on your requirements
  //   const exportPayload = {
  //     pageNumber: 0,
  //     pageSize: 0,
  //     // ... Include the rest of your export payload data
  //   };

  //   // Make an API call to export data here
  //   axios
  //     .post("https://api.aimosa.io/Dashboard/Authors/Export", exportPayload, {
  //       headers: {
  //         Authorization: AuthToken, // Include your authorization token here
  //       },
  //       responseType: "stream", // Receive streamed data
  //     })
  //     .then((response) => {
  ...
  //       const blob = new Blob([stream], { type: "application/csv" });
  ...
  //       "https://api.aimosa.io/Dashboard/Authors/Export"
  //     );
  ...
  //         // Process the streamed data here, e.g., concatenate it
  //         data += new TextDecoder().decode(value);
  //       }

  //       // Once the stream is fully processed, set the converted data
  ...
  const handleExportData = async () => {
    try {
      ...
      a.style.display = "none";
      a.href = url;
      a.download = "Author exported_data.csv";
      document.body.appendChild(a);
      a.click();
      window.URL.revokeObjectURL(url);
              ...
              <Col md={7}>
                <div className="table-footer-right">
                  <Stack spacing={2}>
                    <Pagination
                      count={lastPage}
                      defaultPage={currPage ? currPage : 1}
                      variant="outlined"
                      shape="rounded"
