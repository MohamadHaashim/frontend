  ...
  return (
    <View style={styles.container}>
      <LinearGradient
        ...
        style={isTablet?styles.tabletheaderContainer:styles.headerContainer}
      >
        <View style={styles.header}>
          <LinearGradient
            colors={['rgba(255, 98, 0, 0.4)', 'rgba(72, 0, 172, 0.4)']}
            start={{ x: 0, y: 0 }} end={{ x: 0, y: 1 }}
            style={isTablet?styles.tabletgradientBorder:styles.gradientBorder}
          >
      ...
      {paymentOptions.map((option, index) => (
        <View key={index} style={styles.paymentOptionContainer}>
          ...
          <View style={isTablet?styles.tabletpaymentOption:styles.paymentOption}>
            {option.source && <Image source={option.source} style={styles.icon} />}
            <Text style={isTablet?styles.tablettext:styles.text}>{option.label}</Text>
            ...
            {option.value === 'Bolt' ? (
              disconnectingMethod === option.value ? (
                ...
                <TouchableOpacity onPress={() => handlelinked(option.value)}>
                  <Text style={isTablet?styles.tabletdisconnectText:styles.disconnectText}>Disconnect</Text>
                </TouchableOpacity>
              )
            ) : (
              loadingMethod === option.value ? (
                ...
                <TouchableOpacity onPress={() => handlelinked(option.value)}>
                  <Text style={isTablet?styles.tabletconnectText:styles.connectText}>Connect</Text>
                </TouchableOpacity>
              )
            )}
          </View>
